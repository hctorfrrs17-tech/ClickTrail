...
export function makeStep(payload: Pick<GuideStep, "title" | "targetLabel" | "url" | "screenshot">): GuideStep {
  return {
    ...
    title: payload.title || "Complete this step",
    note: "",
    url: payload.url,
...
export function createGuideHtml(guide: Guide): string {
  const steps = guide.steps.map((step, index) => {
    ...
    return `<article class="step"><div class="step-number">${String(index + 1).padStart(2, "0")}</div><div class="step-body"><h2>${escape(step.title)}</h2>${step.note ? `<p>${escape(step.note)}</p>` : ""}${visual}<a href="${escape(step.url)}" target="_blank" rel="noreferrer">Open the recorded page ↗</a></div></article>`;
  }).join("");
